...
const Notification = ({
  ...
  const acceptFollowRequest = async () => {
    ...
    setNotifications((prev) =>
        prev.map((n) => {
          return n.id === notification.id ? { ...n, type: "FOLLOW", isRead: true } : n;
        })
      );
    await updateFollow(notification.id, notification.actorId, notification.recipientid);
  }
  ...
  return (
    <div className="border-b border-b-zinc-200 p-3 flex flex-2 gap-2 justify-between items-center text-zinc-600">
      ...
      <p className="flex items-center justify-end flex-1 text-zinc-600">
        <span className="mr-2 text-sm">
          {minutesAgo(notification.createdAt)}
        </span>
        {notification.type === "FOLLOW_REQUESTED" && (
          <Button variant="outline" className="mx-2" onClick={acceptFollowRequest}>Accept</Button>
        )}
