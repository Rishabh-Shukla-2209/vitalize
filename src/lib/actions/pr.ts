...
import {changePRFieldQuery, getPersonalRecordsQuery, getPROwner, getRecentPersonalRecordsQuery} from "../queries/pr"
...
export const changePRField = async (
  ...
  const {data, error} = await changePRFieldQuery(userId, prId, exerciseId, newField);
...
export const getPersonalRecords = async (
  ...
  const {data, error} = await getPersonalRecordsQuery(userId, cursor, direction, search);
