      ...
      <div className="flex justify-between p-5 bg-zinc-100 rounded-lg">
        ...
        >
          <div className="flex-center bg-zinc-200 h-15 w-15 rounded-lg">
            <Icons.calendar className="text-primary-dark" size={40} />
          </div>
          <div>
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            <p className="text-zinc-700">Notes: {workout.notes}</p>
            <div className="flex flex-col gap-5">
              {workout.exercises.map((exercise) => (
                <ExerciseLog key={exercise.id} exercise={exercise} name={exercise.exercise.name}/>
              ))}
            </div>
          </motion.div>
        )}
